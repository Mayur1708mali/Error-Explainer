...
Paste an error message or stack trace and get an instant root-cause analysis with concrete fix steps and before/after code examples, powered by a local LLM via [Ollama](https://ollama.com).
...
- **Root-cause analysis** — Identifies the programming language, framework, and explains exactly what went wrong.
- **Fix steps** — Ordered, actionable steps to resolve the error.
- **Code examples** — 1–2 before/after code snippets showing the fix in action, generated by the LLM.
- **Line-numbered input** — Editor-style textarea with a line number gutter for easy reference when discussing stack traces.
- **RAG-powered citations** — Retrieves relevant documentation (MDN, Python docs) and cites real sources.
- **Heuristic + LLM detection** — Fast regex-based language detection with LLM fallback for ambiguous inputs.
- **Analysis history** — Searchable, paginated history of past analyses stored locally.
- **Model selection** — Choose between a fast (3B) or accurate (7B) model in settings.
- **Keyboard shortcuts** — ⌘+Enter to analyze, ⌘+K to search history.
- **Fully local** — All AI processing runs on your machine via Ollama. No data leaves your system.
...
│   │   ├── prompt.ts        # System prompts (incl. code examples instruction)
...
├── src/                     # React frontend
├── shared/
│   ├── schema.ts            # Zod schemas (API contract, incl. CodeExample)
